		...
		if(instance_no == 0 || strcmp(qid, session->query_id) != 0) {
			...
			if(instance_no > 0) {
				...
				if(session->examples.size() > 0) {
					...
					ds->num_examples += session->examples.size();
				}
			}
...
void WeakLearner::learn(Dataset &ds) {
	...
	for(int f = 0; f < ds.num_features; f++) {
		for(int v = ds.min[f]; v < ds.max[f]; v++){
			...
			if(z < bestZ && right[f][v] > wrong[f][v]) {
				bestZ = z;
				bestFeature = f;
				bestTheta = v;
			}
	...
	if(bestFeature > 0)
		...
		cout << "no more positively weighted weak rankers" << endl;
	...
	double delta = 1e-8 + ds.sessions.size()/ds.num_examples;
	this->alpha = 0.5 * log((this->w_right + delta) / (this->w_wrong + delta));
};
